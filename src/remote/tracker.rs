...
impl Tracker {
    ...
    pub fn set_ref(&self, ref_name: &str, ref_value: &str) -> Result<(), lmdb::Error> {
        let env = self.db.env();
        let txn = lmdb::WriteTransaction::new(env)?;

        {
            let mut access = txn.access();
            access.put(&self.db, ref_name, ref_value, lmdb::put::Flags::empty())?;
        }

        txn.commit()
    }

    pub fn get_ref(&self, ref_name: &str) -> Result<Option<String>, lmdb::Error> {
        let env = self.db.env();
        let txn = lmdb::ReadTransaction::new(env)?;
        let access = txn.access();
